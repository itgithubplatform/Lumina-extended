...
// app/api/visualize-lesson/route.ts
// import { NextResponse } from "next/server";
// import { VertexAI } from "@google-cloud/vertexai";
// import { Storage } from "@google-cloud/storage";
...
//   const storage = new Storage({
//     projectId: process.env.GOOGLE_PROJECT_ID,
//     keyFilename: process.env.GOOGLE_APPLICATION_CREDENTIALS || "./vertex_key.json",
//   });
...
//   const timestamp = Date.now();
//   const random = Math.random().toString(36).substring(2, 8);
//   return `slide-${sceneIndex}-${timestamp}-${random}.png`;
// }
...
//     try {
//       const image = await imageModel.generateContent({
//         contents: [{ role: "user", parts: [{ text: `Generate an image for this prompt. Return ONLY the image data, no conversational text. Prompt:${prompt}` }] }],
//       });
...
//       let base64Img = part.inlineData?.data;
//       if (!base64Img) {
//         throw new Error("Base64 data could not be found in inlineData field.");
//       }
...
// // Process slides sequentially with delays to avoid rate limits
// async function processSlidesSequentially(
//   scenes: any[], 
//   imageModel: any
// ): Promise<any[]> {
//   const results = [];
...
//     const prompt = `
// You are an expert educational designer specializing in ADHD-friendly, bite-sized learning materials.
// Transform the input text into EXACTLY 6 sequential learning stops (slides) for a visual interactive focus session.

// Rules:
// 1. Provide EXACTLY 6 slides.
// 2. Each slide must have a fun title with an emoji.
// 3. 'Description' should be clear, engaging prose (2-3 sentences).
// 4. 'Key Idea' should capture the single core takeaway in 1 short sentence.
// 5. 'Image_prompt' should be a vivid descriptive prompt for an educational illustration matching this specific concept.

// Format precisely using this structure for each slide:
// 1. **Title:** [Title with Emoji]
// **Description:** [Text]
// **Key Idea:** [Text]
// **Image_prompt:** [Text]

// Lesson to transform: ${text}
// `;
...
//     const scenes = sceneBlocks.slice(0, 6).map((block, index) => {
//       try {
//         const titleMatch = block.match(/\*\*Title:\*\*(.*?)(?=\*\*Description:)/is);
//         const descriptionMatch = block.match(/\*\*Description:\*\*(.*?)(?=\*\*Key Idea:)/is);
//         const keyIdeaMatch = block.match(/\*\*Key Idea:\*\*(.*?)(?=\*\*Image_prompt:)/is);
...
//           descriptionNormal: descriptionMatch?.[1]?.trim() || '',
//           descriptionSimple: descriptionMatch?.[1]?.trim() || '',
//           keyIdea: keyIdeaMatch?.[1]?.trim() || '',
//           imagePrompt: `${imagePromptMatch?.[1]?.trim() || "educational vector illustration"}, vibrant colors, 512x512, high detail`,
//           sceneNumber: index + 1
//         };
//       } catch (parseError) {
//         return {
//           title: `Slide ${index + 1} 💡`,
//           descriptionNormal: 'Unable to parse text',
//           descriptionSimple: 'Unable to parse text',
//           keyIdea: 'Focus on core concept',
//           imagePrompt: "educational illustration, vibrant colors",
//           sceneNumber: index + 1
//         };
//       }
//     });

//     const processedScenes = await processSlidesSequentially(scenes, imageModel);
...
// Helper function to generate unique file name per slide
function generateFileName(): string {
  const timestamp = Date.now();
  const random = Math.random().toString(36).substring(2, 8);
  return `slide-${timestamp}-${random}.png`;
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!promptToVisualize) {
      return NextResponse.json({ error: "No prompt provided for visualization" }, { status: 400 });
    }
    ...
    console.log(`Generating unique slide image for prompt: ${promptToVisualize.substring(0, 50)}...`);
    
    // Generate image via Vertex AI
    const dataUrl = await generateImageWithRetry(imageModel, promptToVisualize);
    
    // Upload image to storage bucket and get public URL
    ...
    console.error("Error generating visualization image:", err);
    return NextResponse.json({ 
